...
gulp.task('build:es6', ['build-html:es6', 'tsd', 'generate-dts'], function () {
...
gulp.task('build:commonjs', ['build-html:commonjs', 'tsd', 'generate-dts'], function () {
...
gulp.task('build:amd', ['build-html:amd', 'tsd', 'generate-dts'], function () {
...
gulp.task('build:system', ['build-html:system', 'tsd', 'generate-dts'], function () {
  return compileTsProject('system', 'system', 'es5');
});

gulp.task('generate-dts', function(callback) {
  dtsGenerator.default({
    name: paths.pluginName,
    project: './src',
    out: outputDefFile,
    main: paths.pluginName + '/index'
  });
